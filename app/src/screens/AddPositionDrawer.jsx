...
export function AddPositionDrawer({ open, onClose, data }) {
  ...
  const results = q
    ? universe.filter((s) => (s.ticker + s.name).toLowerCase().includes(q.toLowerCase())).slice(0, 6)
    : universe.slice(0, 5);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, pointerEvents: open ? "auto" : "none" }}>
      <div
        onClick={onClose}
        style={{ position: "absolute", inset: 0, background: "rgba(24,32,30,0.32)", opacity: open ? 1 : 0, transition: "opacity var(--dur-base) var(--ease-out)" }}
      />
      <div style={{ position: "absolute", top: 0, right: 0, height: "100%", width: 440, maxWidth: "92vw", background: "var(--surface)", boxShadow: "var(--shadow-xl)", transform: open ? "translateX(0)" : "translateX(100%)", transition: "transform var(--dur-base) var(--ease-out)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "20px 24px", overflow: "auto", flex: 1, display: "flex", flexDirection: "column", gap: 18 }}>
          <div>
            ...
            <Input leadingIcon={<GFIcon name="search" size={16} />} placeholder="Search ticker or name…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14, paddingTop: 4 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[1000, 2500, 5000].map((v) => (
                  <button key={v} onClick={() => setAmount(v)} style={{ flex: 1, padding: "9px 0", border: "1px solid var(--border-strong)", background: amount === v ? "var(--brand-soft)" : "var(--surface)", color: amount === v ? "var(--brand-strong)" : "var(--text-muted)", borderRadius: "var(--radius-md)", cursor: "pointer", font: "inherit", fontSize: 13, fontWeight: 600 }}>${v / 1000}k</button>
                ))}
              </div>
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border)", display: "flex", gap: 10 }}>
          ...
          <Button variant="primary" block disabled={!pick} onClick={onClose}>
