...
import { VerdictPill } from "../components/finance/VerdictPill.jsx";
...
function HalalRow({ r }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "11px 0", borderBottom: "1px solid var(--divider)" }}>
      <span style={{ width: 20, height: 20, borderRadius: "50%", flex: "none", marginTop: 1, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, fontWeight: 700, background: r.pass ? "var(--halal-ok)" : "var(--halal-no)" }}>
        {r.pass ? "✓" : "✕"}
      </span>
      <div>
...
function PositionSizer({ d, onTrade }) {
  ...
  return (
    <Card pad="lg">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[1000, 5000, 10000].map((v) => (
            <button key={v} onClick={() => setAmount(v)} style={{
              ...
              borderRadius: "var(--radius-md)", cursor: "pointer", font: "inherit", fontSize: 13, fontWeight: 600,
            }}>${v / 1000}k</button>
          ))}
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        ...
        <MetricTile label="Proj. 1y" value={<ChangeValue value={11.4} format="currency" showCaret={false} />} sub="at house est. +11.4%" />
      </div>
      <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
        <Button variant="primary" block leadingIcon={<GFIcon name="zap" size={16} />} onClick={() => onTrade && onTrade(d)}>Buy ${amount.toLocaleString()}</Button>
        <Button variant="secondary" leadingIcon={<GFIcon name="star" size={16} />}>Watch</Button>
      </div>
    </Card>
  );
}
  ...
  return (
    <Card pad="lg" style={{ background: "linear-gradient(180deg, var(--evergreen-50), var(--surface))" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        <span style={{ color: "var(--brand)" }}><GFIcon name="sparkles" size={18} /></span>
        <span style={{ fontWeight: 700, color: "var(--text-strong)", fontSize: 15 }}>Ask Grad Fund</span>
      </div>
      <Input
        placeholder={`Ask anything about ${d.name}…`}
        trailingIcon={<span style={{ color: "var(--brand)" }}><GFIcon name="send" size={16} /></span>}
      />
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 12 }}>
        {suggestions.map((s) => (
          <span key={s} style={{ padding: "5px 11px", border: "1px solid var(--border-strong)", borderRadius: "var(--radius-pill)", fontSize: 12.5, color: "var(--text-muted)", background: "var(--surface)", cursor: "pointer" }}>{s}</span>
        ))}
      </div>
    </Card>
  );
...
export function StockDetailScreen({ security, onBack, baseDetail, onTrade }) {
  const d = security && security.ticker === baseDetail.ticker
    ? baseDetail
    : security
    ? { ...baseDetail, ...security, metrics: baseDetail.metrics, halalBreakdown: baseDetail.halalBreakdown, peers: baseDetail.peers, spark: security.spark || baseDetail.spark }
    : baseDetail;
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", flexDirection: "column", gap: 22 }}>
      <button onClick={onBack} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", font: "inherit", fontSize: 13.5, padding: 0, alignSelf: "flex-start" }}>
        <span style={{ transform: "rotate(180deg)", display: "inline-flex" }}><GFIcon name="chevronRight" size={16} /></span> Back to portfolio
      </button>
      ...
      <Tabs
        ...
        items={[
          ...
          { id: "halal", label: "Halal screening", count: 1 },
          { id: "peers", label: "Comparables" },
        ]}
      />
      ...
      {tab === "overview" && (
        <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 22, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
            <PositionSizer d={d} onTrade={onTrade} />
            <AskBox d={d} />
          </div>
        </div>
      )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22, alignItems: "start" }}>
          ...
          <Card pad="lg" style={{ background: "var(--evergreen-50)" }}>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              <span style={{ color: "var(--brand)", marginTop: 2 }}><GFIcon name="shield" size={20} /></span>
              <div>
              ...
              <tr className="gf-tr">
                <td style={{ padding: "12px 0" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <CompanyMark ticker={d.ticker} size={32} />
                    <b style={{ color: "var(--text-strong)" }}>{d.ticker}</b>
                    <Badge tone="brand" size="sm">This</Badge>
                  </div>
                </td>
                <td style={{ textAlign: "center" }}><HalalBadge status={d.halal} showLabel={false} size="sm" /></td>
                <td style={{ textAlign: "right", fontFamily: "var(--font-mono)" }}>31.2</td>
                <td style={{ textAlign: "right", fontFamily: "var(--font-mono)" }}>$2.94T</td>
                <td style={{ textAlign: "right" }}><ChangeValue value={22.6} size="sm" showCaret={false} /></td>
              </tr>
              {d.peers.map((p) => (
                <tr key={p.ticker} className="gf-tr">
                  <td style={{ padding: "12px 0" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <CompanyMark ticker={p.ticker} size={32} />
                      <span>
                        <b style={{ color: "var(--text-strong)" }}>{p.ticker}</b>
