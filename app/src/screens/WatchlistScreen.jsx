...
export function WatchlistScreen({ onOpen, mode = "watchlist", halalOnly, data }) {
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto", display: "flex", flexDirection: "column", gap: 22 }}>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
        ...
        <Button variant="secondary" leadingIcon={<GFIcon name="plus" size={16} />}>New list</Button>
      </div>

      <Card pad="lg">
        <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap", marginBottom: 18 }}>
          <div style={{ display: "flex", gap: 8 }}>
            {TYPES.map((t) => <Chip key={t} active={type === t} onClick={() => setType(t)}>{t === "all" ? "All types" : t}</Chip>)}
          </div>
            ...
            {rows.map((r) => (
              <tr key={r.ticker} className="gf-tr" onClick={() => onOpen(r)} style={{ cursor: "pointer" }}>
                ...
                <td style={{ textAlign: "right", width: 44 }} onClick={(e) => e.stopPropagation()}>
                  <IconButton label="Add to portfolio" variant="ghost"><GFIcon name="plus" size={17} /></IconButton>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {rows.length === 0 && (
          <div style={{ textAlign: "center", padding: "32px 0", color: "var(--text-subtle)", fontSize: 14 }}>No companies match these filters.</div>
        )}
      </Card>
    </div>
  );
