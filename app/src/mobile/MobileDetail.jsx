...
export function MobileDetail({ security, onBack, baseDetail }) {
  const d = security && security.ticker === baseDetail.ticker
    ? baseDetail
    : security
    ? { ...baseDetail, ...security, spark: security.spark || baseDetail.spark }
    : baseDetail;
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--surface)" }}>
      <div style={{ flex: 1, overflow: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px 8px" }}>
          <span onClick={onBack} style={{ color: "var(--text-muted)", display: "inline-flex", cursor: "pointer", transform: "rotate(180deg)" }}>
            <GFIcon name="chevronRight" size={22} />
          </span>
          ...
          <span style={{ color: "var(--text-subtle)" }}><GFIcon name="star" size={22} /></span>
        </div>
        ...
        <div style={{ padding: "8px 18px" }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            ...
            <HalalBadge status={d.halal} size="sm" />
            <AssetTypeBadge type={d.type} size="sm" />
          </div>
        </div>
        ...
        <div style={{ padding: "8px 18px 0" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px 16px" }}>
            {baseDetail.metrics.slice(0, 8).map((m) => <MetricTile key={m.label} label={m.label} value={m.value} />)}
          </div>

          <div style={{ background: "var(--evergreen-50)", borderRadius: "var(--radius-lg)", padding: 16, margin: "20px 0", display: "flex", gap: 10 }}>
            <span style={{ color: "var(--brand)" }}><GFIcon name="shield" size={20} /></span>
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text-strong)" }}>Halal: {d.halal}</div>
      ...
      <div style={{ padding: "12px 18px 26px", borderTop: "1px solid var(--border)", display: "flex", gap: 10 }}>
        ...
        <Button variant="primary" block leadingIcon={<GFIcon name="plus" size={16} />}>Add position</Button>
      </div>
    </div>
  );
}
